...
#define LED_G_OFF (pLEDOUTG = 0)   
#define LED_B_OFF (pLEDOUTB = 0)
#define LED_R_OFF (pLEDOUTR = 0) 

#define LED_G_ON  (pLEDOUTG = 1)
#define LED_B_ON  (pLEDOUTB = 1)
#define LED_R_ON  (pLEDOUTR = 1)


#define OUTPUT_ON  (pOUTPUT = 1)
#define OUTPUT_OFF (pOUTPUT = 0)

#define KEY_PRESS_LED_NOIN    RG_YELLOW		    //无输入时按键按下时状态
#define KEY_RELEASE_LED_NOIN  (~RG_YELLOW)	  //无输入时按键释放时状态

#define KEY_PRESS_LED_IN      LED_RED		      //有输入时按键按下时状态
#define KEY_RELEASE_LED_IN    (~LED_RED)	    //有输入时按键释放时状态
